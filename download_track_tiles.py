...
Coordinates:
    XYZ: Web tiles (top-left origin), TMS: MBTiles (bottom-left origin), Quadkey
...
Module Usage:
    ...
    Example - custom pipeline with filtering:
        ...
        # Buffer in oblique Mercator, transform back to WGS84
        buffered_omerc = buffer_track_metric(line_omerc, 500)
        buffered_wgs84 = transform(transformers.backward.transform, buffered_omerc)
        
        # Enumerate and expand tiles
        tiles = tiles_for_track(buffered_wgs84, 8, 14)
        tiles = apply_corner_expansion(tiles, coords)
